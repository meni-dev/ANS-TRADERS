...
public class CreditNoteItemConfiguration : IEntityTypeConfiguration<CreditNoteItem>
{
    ...
    {
        builder.ToTable("credit_note_items", t => t.HasCheckConstraint(
            "CK_credit_note_items_quantity_positive", "\"Quantity\" > 0"));
        ...
        foreach (var money in new[]
                 {
                     nameof(CreditNoteItem.Rate), nameof(CreditNoteItem.CostRate), nameof(CreditNoteItem.DiscountAmount),
                     nameof(CreditNoteItem.GrossAmount), nameof(CreditNoteItem.TaxableAmount),
                     nameof(CreditNoteItem.CgstAmount), nameof(CreditNoteItem.SgstAmount),
                     nameof(CreditNoteItem.IgstAmount), nameof(CreditNoteItem.LineTotal),
                 })
        {
