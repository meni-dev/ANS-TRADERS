...
public class DebitNoteService : IDebitNoteService
{
            ...
            foreach (var (item, quantity) in lines)
            {
                ...
                if (product is not null)
                {
                    await _stockLedger.EnsureAvailableOnAsync(
                        product, quantity, request.NoteDate, "send back", cancellationToken);
                }
            }

            var financialYear = FinancialYear.For(request.NoteDate);
            var sequence = await _numbers.NextAsync(DocumentKind.DebitNote, financialYear, cancellationToken);
            ...
            foreach (var (item, quantity) in lines)
            {
                ...
                if (product is not null)
                {
                    ...
                    await _stockLedger.RecordAsync(
                        product, -quantity, StockMovementType.PurchaseReturn, note.NoteDate,
    ...
    public async Task CancelAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        foreach (var line in note.Items)
        {
            ...
            {
                await _stockLedger.RecordAsync(
                    product, line.Quantity, StockMovementType.PurchaseReturnCancelled, note.NoteDate,
                    note.Id, note.DebitNoteNumber, notes: null, cancellationToken);
            }
